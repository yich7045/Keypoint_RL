...
class SlacAlgorithm:
    ...
    def __init__(
        ...
        self.latent = LatentModel(state_shape, action_shape, img_feature_dim, z1_dim, z2_dim, hidden_units).to(device)
    ...
    def preprocess(self, ob):
        ...
        tactile = torch.tensor(ob.tactile, dtype=torch.float32, device=self.device).float().div_(1000.)
        action = torch.tensor(ob.action, dtype=torch.float, device=self.device)
        with torch.no_grad():
            feature,_, _ = self.latent.encoder.loss_forward(state, self.Inv_Projection_matrix, self.Inv_Transformation_matrix, tactile, action)
    ...
    def step(self, env, ob, t, is_random):
        ...
        img = state[0][0]
        depth_img = np.round(np.expand_dims(state[0][1], axis=0)*255.).astype(np.uint8)
        if not np.all(depth_img):
            print('issue with depth camera')
        # print(depth_img.dtype)
        img = np.concatenate((img, depth_img))
        tactile = state[1]
        ...
        if done:
            ...
            tactile = state[1]
            action = [0., 0., 0.]
            ob.reset_episode(img, tactile, action)
            self.buffer.reset_episode(img, tactile, action)
    ...
    def update_latent(self, writer):
        ...
        loss_kld, loss_image, loss_reward, sep_loss, s_loss, contact_loss = self.latent.calculate_loss(state_, tactile_, action_, reward_, done_)
        self.optim_latent.zero_grad()
        (loss_kld + loss_image + loss_reward + sep_loss + s_loss + contact_loss).backward()
    ...
    #     alignment_loss= self.latent.calculate_alignment_loss(state_, tactile_, action_)
    #
    #     self.optim_latent.zero_grad()
    ...
    def prepare_batch(self, state_, tactile_, action_):
        with torch.no_grad():
            # f(1:t+1)
            feature_, _, _ = self.latent.encoder.loss_forward(state_, self.Inv_Projection_matrix, self.Inv_Transformation_matrix, tactile_, action_)
            action_ = action_[:, 1:]
            # z(1:t+1)
