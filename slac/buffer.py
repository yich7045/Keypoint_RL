...
class SequenceBuffer:
    ...
    def __init__(self, num_sequences=8):
        ...
        self.action_ = deque(maxlen=self.num_sequences + 1)
    ...
    def reset_episode(self, state, tactile, action):
...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, num_sequences, state_shape, tactile_shape, action_shape, device):
        ...
        self.action_ = torch.empty(buffer_size, num_sequences+1, *action_shape, device=device)
    ...
    def reset_episode(self, state, tactile, action):
        ...
        """
        self.buff.reset_episode(state, tactile, action)
    ...
    def misalign_sample_latent(self, batch_size):
        ...
        idxes_action = np.random.randint(low=0, high=self._n, size=batch_size)

        while len(np.intersect1d(idxes_tactile, idxes_action)) != 0:
            idxes_action = np.random.randint(low=0, high=self._n, size=batch_size)
            idxes_tactile = np.random.randint(low=0, high=self._n, size=batch_size)
        ...
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        tactile_ = torch.tensor(tactile_, dtype=torch.float16, device=self.device).float().div(1000.0)
        return state_, tactile_, self.action_[idxes_action], self.reward_[idxes_state], self.done_[idxes_state]
    ...
    def sample_sac(self, batch_size):
        ...
        for i, idx in enumerate(idxes):
            ...
            tactile_[i, ...] = self.tactile_[idx]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        tactile_ = torch.tensor(tactile_, dtype=torch.float16, device=self.device).float().div(1000.)
